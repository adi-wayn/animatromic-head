...
## 4. Audio Pipeline & Telemetry Protocol
*   **Topology:** The system strictly uses a 4-port UDP topology over local Wi-Fi to prioritize latency:
    *   Control: 4210 (Host -> Edge)
    *   Audio Uplink: 4211 (Edge -> Host)
    *   Audio Downlink: 4212 (Host -> Edge)
    *   Telemetry: 4213 (Edge -> Host)
*   **Dynamic Discovery (No Hardcoded IPs):** The ESP32 and Host must never use hardcoded IP addresses. The Host discovers the ESP32 via mDNS (`animatronic.local`), and the ESP32 latches onto the Host's IP dynamically on every control packet.
