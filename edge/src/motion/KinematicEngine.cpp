/**
 * @file KinematicEngine.cpp
 * @brief Implementation of KinematicEngine.cpp.
 */
...
void KinematicEngine::triggerMove(const ServoConfig& config, double targetAngle, int durationMs,
...
void KinematicEngine::updateKinematics() {
    ...
    for (int i = 0; i < 16; i++) {
        ...
        if (elapsed >= states[i].durationMs) {
            ...
            double easedT = t;
            if (states[i].easingType == EASE_IN_OUT_SINE)
                easedT = easeInOutSine(t);
            else if (states[i].easingType == EASE_IN_OUT_CUBIC)
                easedT = easeInOutCubic(t);
            else if (states[i].easingType == EASE_OUT_EXPO)
                easedT = easeOutExpo(t);

            currentAngle =
                states[i].startAngle + ((states[i].targetAngle - states[i].startAngle) * easedT);
        }

        double delta = currentAngle > currentAngles[i] ? currentAngle - currentAngles[i]
    ...
    for (int i = 0; i < 16; i++) {
        ...
        if (i == NECK_ONE.channel) {
            minA = NECK_ONE.minAngle;
            maxA = NECK_ONE.maxAngle;
        } else if (i == NECK_Y.channel) {
            minA = NECK_Y.minAngle;
            maxA = NECK_Y.maxAngle;
        } else if (i == NECK_ROLL.channel) {
            minA = NECK_ROLL.minAngle;
            maxA = NECK_ROLL.maxAngle;
        } else if (i == EYES_X.channel) {
            minA = EYES_X.minAngle;
            maxA = EYES_X.maxAngle;
        } else if (i == EYES_Y.channel) {
            minA = EYES_Y.minAngle;
            maxA = EYES_Y.maxAngle;
        } else if (i == JAW_UD.channel) {
            minA = JAW_UD.minAngle;
            maxA = JAW_UD.maxAngle;
        } else if (i == JAW_LR.channel) {
            minA = JAW_LR.minAngle;
            maxA = JAW_LR.maxAngle;
        } else if (i == EYELID_LEFT.channel) {
            minA = EYELID_LEFT.minAngle;
            maxA = EYELID_LEFT.maxAngle;
        } else if (i == EYELID_RIGHT.channel) {
            minA = EYELID_RIGHT.minAngle;
            maxA = EYELID_RIGHT.maxAngle;
        }
