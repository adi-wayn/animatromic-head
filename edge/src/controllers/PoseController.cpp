/**
 * @file PoseController.cpp
 * @brief Implementation of PoseController.cpp.
 */
...
void PoseController::executePose(const char* intent) {
    ...
    if (strcmp(intent, "HAPPY") == 0)
        expressHappy();
    else if (strcmp(intent, "SAD") == 0)
        expressSad();
    else if (strcmp(intent, "THINKING") == 0)
        expressThinking();
    else if (strcmp(intent, "ANGRY") == 0)
        expressAngry();
    else if (strcmp(intent, "SURPRISED") == 0)
        expressSurprised();
    else if (strcmp(intent, "NEUTRAL") == 0)
        resetToNeutral();

    // Direct Physical / Primitive Commands
    else if (strcmp(intent, "LOOK_LEFT") == 0)
        lookLeft();
    else if (strcmp(intent, "LOOK_RIGHT") == 0)
        lookRight();
    else if (strcmp(intent, "LOOK_UP") == 0)
        lookUp();
    else if (strcmp(intent, "LOOK_DOWN") == 0)
        lookDown();
    else if (strcmp(intent, "BLINK") == 0)
        blink();
    else if (strcmp(intent, "JAW_OPEN") == 0)
        jawOpen();
    else if (strcmp(intent, "JAW_CLOSE") == 0)
        jawClose();
    else
        resetToNeutral();  // Fallback
}
...
void PoseController::moveJawLR(double targetAngle, int durationMs, EasingType easing) {
    ...
    if (targetAngle < JAW_LR.minAngle)
        targetAngle = JAW_LR.minAngle;
    if (targetAngle > JAW_LR.maxAngle)
        targetAngle = JAW_LR.maxAngle;
...
void PoseController::generateOrganicSaccade(uint32_t timeMs) {
    ...
    double xRange =
        (EYES_X.maxAngle - EYES_X.minAngle) * 0.20;  // 20% physical range for larger X twitches
    double yRange = (EYES_Y.maxAngle - EYES_Y.minAngle) *
                    0.15;  // 15% physical range for more natural up/down movement
    ...
    if (neckNoiseX > 1.0)
        neckNoiseX = 1.0;
    else if (neckNoiseX < -1.0)
        neckNoiseX = -1.0;
    if (neckNoiseY > 1.0)
        neckNoiseY = 1.0;
    else if (neckNoiseY < -1.0)
        neckNoiseY = -1.0;
