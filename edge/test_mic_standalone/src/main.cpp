...
void setup() {
    ...
    i2s_config_t i2s_mic_config = {.mode = (i2s_mode_t)(I2S_MODE_MASTER | I2S_MODE_RX),
                                   .sample_rate = 32000,
...
void loop() {
    ...
    if (err == ESP_OK && bytesRead > 0) {
        ...
        if (barLength > 50)
            barLength = 50;  // Max width of 50 chars
        ...
        // If it crosses your silence threshold (500), mark it with a different character or just
        // show a long bar
        for (int i = 0; i < 50; i++) {
            if (i < barLength) {
                if (rms > 500)
                    Serial.print("#");  // Loud enough to trigger system
                else
                    Serial.print("=");  // Just background noise
            } else {
                Serial.print(" ");
            }
        }
