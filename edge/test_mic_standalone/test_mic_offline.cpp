...
void setup() {
    ...
    while (samplesRead < NUM_SAMPLES) {
        ...
        for (int i = 0; i < frames; i++) {
            if (samplesRead >= NUM_SAMPLES) break;
    ...
    for (int i = 0; i < NUM_SAMPLES; i++) {
        ...
        Serial.print(",");
        if (i % 20 == 19) Serial.println(); // 20 samples per line
    }
    
    Serial.println();
    Serial.println("---AUDIO_END---");
    Serial.println("[Test] Dump complete. Please copy everything between AUDIO_START and AUDIO_END.");
}
