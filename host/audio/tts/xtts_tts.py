...
class XTTSStrategy(TTSStrategy):
    ...
    def synthesize_stream(self, text: str):
        """
        Yields raw PCM audio bytes in chunks as soon as they are generated by XTTS.
        """
        self._is_interrupted = False
        ...
        try:
            # We precompute the speaker embeddings to avoid recomputing on every call
            if not hasattr(self, 'speaker_embedding'):
                logger.info("Precomputing XTTS speaker latents...")
                self.gpt_cond_latent, self.speaker_embedding = self.tts.synthesizer.tts_model.get_conditioning_latents(audio_path=[self.reference_audio])
                
            logger.debug(f"XTTS deep streaming audio for: {text}")
            ...
            for chunk in chunks:
                ...
                # Wrap it in a quick WAV header so dual_tts_manager can process it transparently
                wav_io = io.BytesIO()
                with wave.open(wav_io, 'wb') as wav_file:
                    wav_file.setnchannels(1)
                    wav_file.setsampwidth(2)
            ...
            logger.error(f"XTTS streaming generation failed: {e}")
            yield b""
