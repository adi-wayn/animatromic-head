"""Coqui XTTS v2 generation strategy."""
...
class XTTSStrategy(TTSStrategy):
    ...
    def __init__(self, reference_audio: str = "assets/short_reference.wav"):
    ...
    def synthesize(self, text: str) -> bytes:
        ...
        if not os.path.exists(self.reference_audio):
            logger.warning(
                f"Reference audio {self.reference_audio} not found. Returning empty bytes."
            )
            return b""
        ...
        try:
            ...
            wav_list = self.tts.tts(text=text, speaker_wav=self.reference_audio, language="en")
    ...
    def synthesize_stream(self, text: str):
        """
        Yields the full sentence to avoid scipy resampling artifacts on tiny chunks.
        """
        self._is_interrupted = False
        if not self.tts or not os.path.exists(self.reference_audio):
            yield b""
            return

        try:
            logger.debug(f"XTTS full-sentence synthesis for: {text}")
            audio_bytes = self.synthesize(text)
            if not self._is_interrupted and audio_bytes:
                yield audio_bytes
        except Exception as e:
