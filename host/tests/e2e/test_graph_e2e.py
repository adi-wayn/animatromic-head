...
from core.graph import build_graph, CognitiveOutput
...
async def test_graph_routing_e2e():
    ...
    with patch("core.graph.LLMManager") as mock_manager, \
         ...
         patch("core.graph.broadcast_phase") as mock_broadcast, \
        ...
        mock_structured.ainvoke = AsyncMock(return_value=CognitiveOutput(emotion="HAPPY", response_text="Hi"))
        ...
        async for event in graph.astream({"messages": []}, config):
            ...
            if visited_nodes[-1] == "listen_node" and graph_module.text_queue.empty() and "interrupt_node" in visited_nodes:
                break
