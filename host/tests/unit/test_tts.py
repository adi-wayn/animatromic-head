...
def test_dual_tts_manager_speak_success(mock_socket, mock_xtts_class):
    ...
    with patch.object(manager, "_play_with_lip_sync", wraps=manager._play_with_lip_sync) as mock_play:
        with patch("audio.tts.dual_tts_manager._adapter") as mock_adapter:
