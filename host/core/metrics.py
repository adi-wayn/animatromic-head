...
class TurnMetrics:
    ...
    def mark_stt_end(self):
        ...
        if self.t_vad_end > 0:
            logger.info(f"⏱️ [METRICS] STT Transcription took: {self.t_stt_end - self.t_vad_end:.3f}s")
    ...
    def mark_tts_start(self):
        ...
        if self.t_llm_end > 0:
            logger.info(f"⏱️ [METRICS] TTS Generation took: {self.t_tts_first_byte - self.t_llm_end:.3f}s")
            
        if self.t_vad_end > 0:
            total = self.t_tts_first_byte - self.t_vad_end
            logger.info(f"🚀 [METRICS] === TOTAL LATENCY (User stopped speaking -> First Sound played) === : {total:.3f}s")
