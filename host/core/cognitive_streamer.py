...
async def process_cognitive_stream(llm_stream) -> tuple[str, str]:
    ...
    async for chunk in llm_stream:
        ...
        if hasattr(chunk, "emotion") and getattr(chunk, "emotion") and not emotion_dispatched:
        ...
        current_text = getattr(chunk, "response_text", "") if hasattr(chunk, "response_text") else (chunk.get("response_text", "") if isinstance(chunk, dict) else "")
        if not current_text:
            continue
        ...
        if diff:
            ...
            while True:
                first_delim = min([buffer.find(d) for d in sentence_delimiters if buffer.find(d) != -1], default=-1)
                if first_delim != -1:
                    ...
                    if len(sentence) > 1:
                        ...
                        # We use print here intentionally to create a typewriter effect for the AI's response in the terminal.
