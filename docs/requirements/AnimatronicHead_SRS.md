...
*   **SR-KIN-001 (Complex Pose Generation)**: The physical response MUST NOT be limited to atomic emotions (e.g., "Happy"). The controller MUST interpret rich contextual scenarios. For example, "Happy" MUST execute a synchronized macro: tilting the neck up, opening the eyes wide, and slightly opening the jaw to form a smile.
