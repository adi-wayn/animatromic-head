...
### 6.2 The Unified Protocol Envelope (Host <-> Edge)
To ensure the Edge (C++) and Host (Python) systems are completely decoupled and parse messages identically, all state and command JSON payloads MUST adhere to a strict standard Envelope. This envelope is formalized in Python using `Pydantic` models inside the `host/protocol/` module.
...
### 6.3 Valid Protocol Payload Types
The ESP32 JSON parser MUST route execution based purely on the Envelope's `"type"` string.

#### 1. The `INTENT` Payload
Generated by the LLM (or autonomous graph) to command physical movement.
```json
{
  "type": "INTENT",
  "payload": {
...
```
*Architectural Scalability:* The developer simply updates the ESP32's `PoseDictionary` to map new "emotion_primary" strings to newly installed servos.

#### 2. The `PHASE_UPDATE` Payload
Informs the Edge of the Host's conversational state (`LISTENING`, `MOVING`, `SPEAKING`). Used to coordinate saccades and idle behaviors.
```json
{
  "type": "PHASE_UPDATE",
  "payload": {
...
#### 3. The `EMERGENCY_STOP` Payload
Fired by the WebRTC VAD to immediately halt speech and movement.
```json
{
  "type": "EMERGENCY_STOP",
  "payload": {}
